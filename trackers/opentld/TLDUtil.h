...
void tldExtractNormalizedPatch(const cv::Mat& img, int x, int y, int w, int h,
                               float *output);
void tldExtractNormalizedPatchBB(const cv::Mat& img, int *boundary,
                                 float *output);
void tldExtractNormalizedPatchRect(const cv::Mat& img, cv::Rect *rect,
                                   float *output);
void tldExtractSubImage(const cv::Mat& img, cv::Mat& subImage, int *boundary);
void tldExtractSubImage(const cv::Mat& img, cv::Mat& subImage, int x, int y,
                        int w, int h);

float tldCalcMean(float *value, int n);
float tldCalcVariance(float *value, int n);

bool tldSortByOverlapDesc(std::pair<int, float> bb1 ,
                          std::pair<int, float> bb2);
cv::Rect *tldCopyRect(cv::Rect *r);

//TODO: Change function names
float tldOverlapRectRect(cv::Rect r1, cv::Rect r2);
void tldOverlapOne(int *windows, int numWindows, int index,
                   std::vector<int> * indices, float *overlap);
void tldOverlap(int *windows, int numWindows, int *boundary, float *overlap);
void tldOverlapRect(int *windows, int numWindows, cv::Rect *boundary,
                    float *overlap);
