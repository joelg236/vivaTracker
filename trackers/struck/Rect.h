...
class STRUCKRect {
  public:
    ...
    template <typename T2>
    friend std::ostream& operator <<(std::ostream& rOS,
                                     const STRUCKRect<T2>& rRect);
...
std::ostream& operator <<(std::ostream& rOS, const STRUCKRect<T>& rRect) {
    rOS << "[origin: (" << rRect.m_xMin << ", " << rRect.m_yMin << ") size: (" <<
        rRect.m_width << ", " << rRect.m_height << ")]";
    return rOS;
}
...
bool STRUCKRect<T>::IsInside(const STRUCKRect<T2>& rOther) const {
    return (XMin() >= rOther.XMin()) && (YMin() >= rOther.YMin())
           && (XMax() <= rOther.XMax()) && (YMax() <= rOther.YMax());
}
